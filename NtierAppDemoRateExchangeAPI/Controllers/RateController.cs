...
using NtierAppDemo.Bootstrapper;
using NtierAppDemo.Core.Exceptions;
using NtierAppDemo.Core.Services.Contracts;
using NtierAppDemoRateExchangeAPI.Models;

namespace NtierAppDemoRateExchangeAPI.Controllers
{
    public class RateController : ApiController
    {
        ...
        public RateResponse GetRate(RequestModel model)
        {
            ...
            if(rate==null)
            {
                return new RateResponse() { err = "No data fount", returncode = 404, timestamp = DateTime.UtcNow.Ticks };
            }
